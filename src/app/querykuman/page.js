...
const Query = () => {
    ...
    try {
      ...
      if (!response.ok) {
        if (backupData && backupData.error) {
          ...
          setError("Gagal mengambil antibiotik terbaik dari API cadangan.");
        }
        setResults(null);
        setDataSource(null);
        return false;
      ...
      if (!backupData || !backupData.bakteri || !Array.isArray(backupData.tiga_antibiotik)) {
        setError("Data dari API cadangan tidak valid.");
        setResults(null);
        setDataSource(null);
        return false;
      }
      ...
      setResults({
        ...
        tiga_antibiotik: backupData.tiga_antibiotik.slice(0, 5).map((item) => ({
          ...
          Score:
            item[backupData.bakteri] !== undefined
              ? item[backupData.bakteri]
  ...
  const getFilteredSuggestions = () => {
    ...
    return bacteriaSuggestions
      .filter((suggestion) => suggestion.toLowerCase().includes(trimmedInput))
      .slice(0, 5);
  };
            ...
            <div className={styles.results}>
              ...
              {dataSource === "firebase" && (
                <div className={styles.info}>
                  <p>Data diambil dari database utama (Firestore).</p>
                </div>
              )}
            </div>
          )}
